...
//Class Constructor
	function __construct() {
		...
		if( $this->isEnabled() ) {
			...
			$this->sort_order = MODULE_SHIPPING_FEDEXWEBSERVICES_SORT_ORDER;
			$this->tax_class = MODULE_SHIPPING_FEDEXWEBSERVICES_TAX_CLASS;
		}
	}
	//Class Methods
	function quote( $pShipHash = array() ) {
		...
		if ((MODULE_SHIPPING_FEDEXWEBSERVICES_GROUND == 'true' && $order->delivery['country']['countries_id'] == STORE_COUNTRY) || (MODULE_SHIPPING_FEDEXWEBSERVICES_GROUND == 'true' && ($order->delivery['country']['countries_id'] != STORE_COUNTRY) && MODULE_SHIPPING_FEDEXWEBSERVICES_INTERNATIONAL_GROUND == 'true')) {
			$this->types['FEDEX_GROUND'] = array( 'code' => 'FEDEX_GROUND', 'icon' => '', 'handling_fee' => ($order->delivery['country']['countries_id'] == STORE_COUNTRY ? MODULE_SHIPPING_FEDEXWEBSERVICES_HANDLING_FEE : MODULE_SHIPPING_FEDEXWEBSERVICES_INT_HANDLING_FEE));
			$this->types['GROUND_HOME_DELIVERY'] = array( 'code' => 'FEDEX_GROUND_HOME_DELIVERY', 'icon' => '', 'handling_fee' => ($order->delivery['country']['countries_id'] == STORE_COUNTRY ? MODULE_SHIPPING_FEDEXWEBSERVICES_HANDLING_FEE : MODULE_SHIPPING_FEDEXWEBSERVICES_INT_HANDLING_FEE));
		}
		...
		$request['RequestedShipment']['Shipper'] = array(	'Address' => array(
															'StreetLines' => array(MODULE_SHIPPING_FEDEXWEBSERVICES_ADDRESS_1, MODULE_SHIPPING_FEDEXWEBSERVICES_ADDRESS_2), // Origin details
															'City' => MODULE_SHIPPING_FEDEXWEBSERVICES_CITY,
															'StateOrProvinceCode' => MODULE_SHIPPING_FEDEXWEBSERVICES_STATE,
															'PostalCode' => MODULE_SHIPPING_FEDEXWEBSERVICES_POSTAL,
		...
		if (MODULE_SHIPPING_FEDEXWEBSERVICES_SIGNATURE_OPTION >= 0 && $totals >= MODULE_SHIPPING_FEDEXWEBSERVICES_SIGNATURE_OPTION) { 
	...
	function isInstalled() {
		if( $ret = parent::isInstalled() ) {
			if( $this->getConfig( 'MODULE_SHIPPING_FEDEXWEBSERVICES_VERSION' ) != '1.3.0') {
				$this->storeConfig( 'MODULE_SHIPPING_FEDEXWEBSERVICES_VERSION', '1.3.0' );
			}
		}
		return $ret;
	}
	function install() {
		$this->mDb->query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Enable FedEx Web Services','MODULE_SHIPPING_FEDEXWEBSERVICES_STATUS','true','Do you want to offer FedEx shipping?','6','0','zen_cfg_select_option(array(''true'',''false''),',now())");
		...
		$this->mDb->query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Shipping Zone', 'MODULE_SHIPPING_FEDEXWEBSERVICES_ZONE', '0', 'If a zone is selected, only enable this shipping method for that zone.', '6', '98', 'zen_get_zone_class_title', 'zen_cfg_pull_down_zone_classes(', now())");
		$this->mDb->query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Tax Class', 'MODULE_SHIPPING_FEDEXWEBSERVICES_TAX_CLASS', '0', 'Use the following tax class on the shipping fee.', '6', '25', 'zen_get_tax_class_title', 'zen_cfg_pull_down_tax_classes(', now())");
		$this->mDb->query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_SHIPPING_FEDEXWEBSERVICES_SORT_ORDER', '0', 'Sort order of display.', '6', '99', now())"); 
	}
	function remove() {
		$this->mDb->query("DELETE FROM ". TABLE_CONFIGURATION ." WHERE configuration_key in ('". implode("','",$this->keys()). "')");
	}
	function keys() {
		return array('MODULE_SHIPPING_FEDEXWEBSERVICES_STATUS',
					 ...
					 'MODULE_SHIPPING_FEDEXWEBSERVICES_TAX_CLASS',
					 ...
					 'MODULE_SHIPPING_FEDEXWEBSERVICES_ZONE',
					 'MODULE_SHIPPING_FEDEXWEBSERVICES_SORT_ORDER'
					 );
	}
}